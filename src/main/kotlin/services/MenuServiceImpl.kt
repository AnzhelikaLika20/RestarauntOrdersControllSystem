...
class MenuServiceImpl(private val dishRepository: DishRepositoryImpl) : MenuService {
    ...
    override fun getAvailableDishes(): List<Dish> {
        return dishRepository.getAvailableDishes()
    }

    override fun getDishByName(dishName: String): DishResponse {
        val dish = dishRepository.getDishByName(dishName) ?: return DishResponse(
            ResponseCode.BadRequest,
            "Dish $dishName is unknown, so it would not be added to the order",
            null
        )
        return DishResponse(ResponseCode.Success, "Dish $dishName was added into order\n\n", dish)
    }
}